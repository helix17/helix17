# 👋 Hi, I'm Hemachandran Balaji

🎓 Master's Student in Computer Science @ **UC Davis**  
💻 Aspiring Software Engineer | Systems Programming Enthusiast | Cloud Technology Explorer

----

### 🚀 About Me

I am passionate about building efficient, scalable, and high-performance software solutions. Currently pursuing my Master’s in Computer Science at UC Davis, I have a strong background in **C++, cloud technologies, systems programming, and machine learning**.

I love solving complex problems, optimizing system performance, and contributing to impactful projects.

- 🔭 **Currently working on:** Performance analysis using gem5 and SRGAN optimization with PatchGAN.
- 🌱 **Learning:** Advanced systems programming and cloud.
- 💡 **Interests:** systems, high-performance computing, and cloud infrastructure.
- ⚡ **Fun fact:** I enjoy breaking down complex algorithms into simple, efficient code that doesn't work! JK.

---

### 🛠️ Tech Stack

**Languages:** `C++` `Python` `SQL` `JavaScript` `HTML` `CSS`

**Frameworks:** `React` `NumPy` `PyTorch`

**Tools:** `AWS` `Git` `MySQL` `Arduino IDE`

---

### 📌 Featured Projects

- **[Optimizing Wind Farm Placement](https://github.com/helix17/CA-potential-wind-capacity-analysis)**  
  *Data analysis project identifying optimal wind farm locations using geospatial data processing and [visualizing potential windfarms](https://helix17.github.io/CA-potential-wind-capacity-analysis/)*

- **[SRGAN Enhancement with PatchGAN](https://github.com/helix17/Patch-SRGAN-KD)**  
  *Super-resolution model fine-tuned with PatchGAN for improved image quality using the DIV2K dataset.*

- **[IoT-Based Contactless Elevator System](https://github.com/helix17/)**  
  *Developed an Arduino-powered contactless keypad using ultrasonic sensors for improved hygiene and accessibility.*

---

Feel free to explore my repositories, reach out for collaborations, or just say hi! 👋

![Profile Views](https://komarev.com/ghpvc/?username=helix17&color=blue)


<!--
**helix17/helix17** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...


### 📈 GitHub Stats

![Hemachandran's GitHub Stats](https://github-readme-stats.vercel.app/api?username=helix17&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=helix17&layout=compact&theme=radical)

---
-->
